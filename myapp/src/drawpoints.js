...
function distance(point1, point2) {
    ...
    return Math.sqrt((point1.x - point2.x) ** 2 + (point1.y+500 - point2.y) ** 2);
  }
  const comp=(p1,p2)=>{
...
const Drawpoints=()=>{
    ...
    useEffect(()=>{
        ...
        if(len>1)
        {
          ...
          setconnection(prevconnnection=>([...prevconnnection,{startPoint,endPoint}]))
        }
      },[newgrid])
        ...
        if (event.button === 0) { // Check if left mouse button is clicked
          ...
          for (const existingPoint of coordinatesArray) {
            ...
            if (distance(existingPoint, newPoint) < 20) { // Adjust the distance threshold as needed
             newPoint.x=existingPoint.x;newPoint.y=existingPoint.y+500;
...
return <React.Fragment>
    <div onClick={handleClick} style={{ height: '300vh', position: 'relative' }}>
